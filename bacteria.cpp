...
bool Bacteria::canEat(Food& f) {
    if ((position - f.get_position()).length() < radius) {
        food_collected += f.nutrition_value;
        f.is_eaten = true;
    }
    return  ((position - f.get_position()).length() < radius);
}
...
void Bacteria::active_motion(evol::Vector tl_b, evol::Vector br_b) {
    ...
    while (boundary_flag != 0) { /** Ели выходит: */
        ...
        dmov = first_bound_cross(br_b, tl_b); /** Узнаём, сколько она пройдёт до ближайшей
                                                ...
                                                */
        position.x += dmov * cos(angle); /** Передвигаем частицу до этой границы */
...
//    f.set_boundaries(w);
    std::cout << "Bacteria's position is: (" << b1.position.x << ", " << b1.position.y <<"), it's angle is " << b1.angle / M_PI <<" PI\n";
    ...
    std::cout << "Bacteria has " << b1.food_collected << " nutritions\n";
    ...
    b1.step();
    std::cout << "Bacteria moved to (" << b1.position.x << ", " << b1.position.y <<"), it's angle changed to " << b1.angle << "\n";

    Bacteria b({1.0, 2.0}, 2.0, 5 * M_PI / 4, 6.0);
    std::cout << "\nNew bacteria's position is: (" << b.position.x << ", " << b.position.y <<"), it's angle is " << b.angle / M_PI <<" PI\n";
    b.active_motion(b.top_left, b.bottom_right);
    std::cout << "Bacteria actioned to (" << b.position.x << ", " << b.position.y <<"), it's angle changed to " << b.angle / M_PI << " PI\n";
    b.brownian_motion();
    std::cout << "Bacteria b_moved to (" << b.position.x << ", " << b.position.y <<"), it's angle changed to " << b.angle / M_PI << " PI\n";
